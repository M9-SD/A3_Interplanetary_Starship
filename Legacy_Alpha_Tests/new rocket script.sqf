...
M9SD_fnc_rocketAssembly = {
	...
	private _rocketFuselage = createSimpleObject ["ammo_Missile_mim145", _pos];
	[_rocketFuselage,[_rocketBaseObj,[0,0,241.195]]] remoteExec ['attachTo'];
	private _y = 0;
	private _p = -90;
	private _r = 90;
	[
	...
	[_rocketFuselage, 100] remoteExec ['setObjectScale', 0, _rocketFuselage];
	_rocketBaseObj;
};
	...
	0 = [_this] spawn {
		...
		(_rocketObj_stage_01 getVariable ['enginesFiring', false]))} do {
			...
			if !(_upVec isEqualTo _straightUp) then {
				...
				if ((speed _rocketObj_stage_01 >= _soundBarrier) or (((velocityModelSpace _rocketObj_stage_01 # 2) * 3.6) >= _soundBarrier)) then {
