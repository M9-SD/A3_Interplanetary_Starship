...
M9SD_fnc_rocketDescent = {
	...
	0 = [_this] spawn {
		...
		private _getDescentSpeed = {
			...
			_v = _v / 3.6;
			_v = 277.778 - _v;
			systemChat format ['_v = %1', _v];
			_v;
		};
		
		systemChat "control start";
		...
		(_rocketObj_stage_01 getVariable ['enginesFiring', false]) && (!isTouchingGround _rocketObj_stage_01))} do {
			...
			systemChat format ['_altitude = %1', _altitude];
			...
			if (_altitude <= 1000) then {
				...
				systemChat format ['_downVel = %1', _downVel];
				private _vel = [0,0,_downVel];
				systemChat format ['_vel = %1', _vel];
...
M9SD_fnc_initRocketSequence_landing = {
	...
	uisleep 0.5;
	private _pos = getPosATL _rocket;
	...
	while {(getPosATL _rocket # 2) > 1000} do {
		_rocket setVelocityModelSpace [0,0,-277.778];
	};
	_rocket spawn {
