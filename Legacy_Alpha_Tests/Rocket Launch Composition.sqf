...
comment "set the view distance manually";
...
M9SD_fnc_rocketAscent = {
	...
	0 = [_this] spawn {
		...
		M9_tmpREfnc_sv = ['', _fncsv];
		publicVariable 'M9_tmpREfnc_sv';
		...
		(_rocketObj_stage_01 getVariable ['enginesFiring', false]))} do {
			...
			[[_rocketObj_stage_01, _vel],{_this call (M9_tmpREfnc_sv # 1)}] remoteExec ['call'];
